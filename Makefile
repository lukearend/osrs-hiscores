...
init: db-pull env nbextensions lint
...
$(DATA_RAW)/usernames-raw.csv:
	@source env/bin/activate && \
	cd src/data && python3 scrape_usernames.py $@ && \
	touch $@
...
$(DATA_RAW)/stats-raw.csv: $(DATA_TMP)/usernames.csv
	@source env/bin/activate && \
	cd src/data && python3 scrape_stats.py $< $@ && \
	touch $@
...
scrape-clean:       ## Remove processed scraped data (but just touch raw files).
	touch $(DATA_RAW)/usernames-raw.csv
	touch $(DATA_RAW)/stats-raw.csv
	rm -f $(DATA_TMP)/usernames.csv
	rm -f $(DATA_FINAL)/stats.csv

.PHONY: scrape scrape-touch scrape-clean scrape-clobber
...
analytics-clean:    ## Remove all analytic results computed from scraped data.
...
app-clean:          ## Remove all application data including database.
app-clean: db-stop
...
db-start:           ## (Re)start database service.
